...
4. Preserve proper nouns, product names, code, formulas, URLs, citations, numbers, and placeholders unless they have a standard localized form.
5. Never summarize, explain, censor, expand, or invent information. Translate every meaningful part of the input.
6. Keep terminology consistent across the document and prefer natural native phrasing over literal word order.
...
4. Keep each output segment concise enough to read at the original speaking pace. Remove verbal filler only when it carries no meaning.
5. Preserve proper nouns, numbers, jokes, speaker intent, and repeated words that are meaningful.
6. Never summarize, explain, merge separate segments, add labels, or invent missing context.
