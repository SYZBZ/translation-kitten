...
export class UniversalVideoAdapter {
  ...
  /** Reattach controls after YouTube replaces the player DOM without a full navigation. */
  async ensureMounted() {
    const video = this.subtitlesScheduler?.getVideoElement()
    const button = document.getElementById(TRANSLATE_BUTTON_CONTAINER_ID)
    if (video?.isConnected && button?.isConnected)
      return

    await this.renderTranslateButton()
  ...
  private async startTranslation(analyticsContext?: FeatureUsageContext) {
    ...
    const isCurrent = () => generation === this.translationGeneration
    try {
      const currentVideoId = this.config.getVideoId?.() ?? ""
