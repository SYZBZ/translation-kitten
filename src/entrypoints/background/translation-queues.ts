...
import { getPdfTranslatePrompt } from "@/utils/prompts/pdf"
...
export async function setUpPdfTranslationQueue() {
  const config = await ensureInitializedConfig()
  const { translate: { requestQueueConfig, batchQueueConfig } } = config ?? DEFAULT_CONFIG
  const { requestQueue, batchQueue } = await createTranslationQueues({
    requestQueueConfig,
    batchQueueConfig,
    promptResolver: getPdfTranslatePrompt,
  })

  onMessage("enqueuePdfTranslateRequest", async (message) => {
    const { text, langConfig, providerConfig, scheduleAt, hash } = message.data
    const cached = hash ? await db.translationCache.get(hash) : undefined
    if (cached)
      return normalizeTranslationOutput(providerConfig, cached.translation)

    const result = shouldUseBatchQueue(providerConfig)
      ? await batchQueue.enqueue({ text, langConfig, providerConfig, scheduleAt, hash })
      : await requestQueue.enqueue(
          () => executeTranslate(text, langConfig, providerConfig, getPdfTranslatePrompt),
          scheduleAt,
          hash,
        )
    ...
    if (normalized && hash) {
      await db.translationCache.put({ key: hash, translation: normalized, createdAt: new Date() })
    }
    return normalized
  })
}
