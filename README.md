...
以 [Read Frog](https://github.com/mengxi-ream/read-frog) 為基礎的個人 Chrome 翻譯擴充功能，新增 PDF 雙欄翻譯本、原文／譯文同步選取標示，並強化 YouTube 字幕按鈕在換片與播放器重建後的回應。

## 本機安裝

1. 安裝 Node.js 與 pnpm，執行 `pnpm install`。
2. PowerShell 執行 `$env:WXT_SKIP_ENV_VALIDATION='true'; pnpm build`。
3. Chrome 開啟 `chrome://extensions`，啟用「開發人員模式」。
4. 點擊「載入未封裝項目」，選擇 `.output/chrome-mv3`。
5. 若要直接開啟本機 PDF，在擴充功能詳細資料中啟用「允許存取檔案網址」。也可在 PDF 翻譯本內用檔案選擇器開啟。

PDF 分頁中點擊擴充功能，再選「開啟這份 PDF 的翻譯本」。YouTube 支援設定成偵測字幕後自動開始，或使用播放器內的常駐小貓按鈕手動開始。免費翻譯與 API Key 供應商均沿用；API Key 只儲存在 Chrome storage。

## 授權與來源

本專案是 Read Frog 的 GPL-3.0 衍生作品，保留原始 `LICENSE` 與作者資訊。下方為上游專案說明。
